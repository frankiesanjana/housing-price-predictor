...
from src.data_management import (
    load_housing_data, load_inherited_houses_data, load_pkl_file
)
...
def page_sale_price_predictor_body():
    """
    Displays the predicted sale price for inherited homes
    and allows prediction on live input data
    """
    # load the required files and create df for inherited properties
    """
    NB: a number of the following lines produce "E501 line too long" errors
    in Python linter checks
    However, in the interest of human readability, they remain this way
    """
    version = 'v1'
    ...
    st.info(
       ...
       f"prices from her 4 inherited houses, and any other house in Ames, "
       f"Iowa."
    )
    ...
    st.write(
        f"* We then subset the most relevant features to be used for sale "
        f"price prediction, and display the predicted sale price for each "
        f"house:"
    )
    df_inherited_filtered = df_inherited_houses.filter(house_features)
    sale_price_prediction = np.round(sale_price_pipeline.
                                     predict(df_inherited_filtered), 0)
    df_inherited_filtered["Predicted Sale Price"] = sale_price_prediction

    st.write(df_inherited_filtered.head())

    summed_sale_price = "${:,.0f}".format(
        df_inherited_filtered["Predicted Sale Price"].sum()
        )
    ...
    st.info(
        f"### Predicting sales price of any other house in Ames, Iowa "
        f"(BR2, part 2)"
        )
    st.info(
       f"* **BR2** - The client is interested in predicting the house sale "
...
def draw_input_widgets():
    ...
    # we create an empty DataFrame, which will be the live data
    ...
    with col2:
        ...
        st_widget = st.number_input(
            ...
            # set max to 2100, since by then our model will likely be obsolete
            max_value=int(2100),
