...
def page_predict_price_ml_body():
    ...
    st.info(
        f"* To fulfil BR2, we trained a regressor model to predict the sale "
        f"price of properties in Ames, Iowa based on a given set of "
        f"attributes. The pipeline was tuned aiming to ensure an R2 score "
        f"of at least 0.75 on both train and test sets.\n"
        f"* The pipeline performance for the best model on the train and test "
        f"set is an R2 score of 0.97 and R2 0.78 respectively, so it meets "
        f"the performance requirement indicated in the business case.\n"
    ...
    st.info(
        ...
        f"generally able to provide an acceptable indication of the sale "
        f"price of a house based on the input features, although we do note "
        f"that the model appears somewhat less effective for higher value "
        f"houses."
        )
